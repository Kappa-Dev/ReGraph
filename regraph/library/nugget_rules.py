...
class AbstractRule():
    ...
    def get_nugget(self, matching, exceptions):
        ...
        # is the new nugget an exception
        possible_exceptions = (e for e in exceptions
                               if (len(e.nodes()) == len(new_nugget.nodes()) and
                                   len(e.edges()) == len(new_nugget.edges())))
        for e in possible_exceptions:
            pattern = deepcopy(e)
            #pattern.metamodel_ = new_nugget.metamodel_
            new_nugget.metamodel_ = None
            print("before")
            instances = Rewriter.find_matching(new_nugget, pattern)
            print("after")
            if instances:
                return None

        # we correct the meta model if no exception was found
        for (n1, n2) in new_nugget.edges():
            tn1 = new_nugget.node[n1].type_
            tn2 = new_nugget.node[n2].type_
    ...
    def get_nuggets(self, exceptions):
        matchings = self.get_matchings()
        nuggets = [self.get_nugget(m, exceptions) for m in matchings]
        return [n for n in nuggets if n is not None]


class AbstractRules():
    # pre: metamodels must be the same
    def __init__(self, rules, pattern_key, exceptions):
        if rules:
    ...
    def unfold_once(self):
        ...
        for (name, ARule) in self.abstract_rules:
            nuggets = ARule.get_nuggets(self.exceptions)
...
#     # precondition: metamodels must be the same
