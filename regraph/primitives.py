...
def add_edge(graph, s, t, attrs=None, **attr):
    ...
    if isinstance(graph, nx.DiGraph):
        ...
        graph.add_edge(s, t)
        graph.edges[s, t] = new_attrs
        graph.edges[t, s] = new_attrs
    elif isinstance(graph, Neo4jGraph):
...
def merge_nodes(graph, nodes, node_id=None, method="union", edge_method="union"):
    ...
    if len(nodes) == 1:
        ...
        if isinstance(graph, Neo4jGraph) and\
           (method == "intersection" or edge_method == "intersection"):
            raise ReGraphError(
                "Mergining nodes with intersection of "
                "attributes is not available for Neo4jGraph!")
